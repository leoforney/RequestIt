...
public class SearchController {
    ...
    public tk.leoforney.requestit.model.spotify.Track getTrackById(SpotifyAuthorization authorization, String id) {
        SpotifyApi spotifyApi = getSpotifyApi(authorization);
        return null;
    }

    public List<tk.leoforney.requestit.model.spotify.Track> getTracksInPlaylist(SpotifyAuthorization authorization, PlaylistSimplified playlist) {
        SpotifyApi spotifyApi = getSpotifyApi(authorization);
        ...
        try {
            ...
            List<tk.leoforney.requestit.model.spotify.Track> tracksConverted = new ArrayList<>();
                ...
                for (PlaylistTrack track : pts.getItems()) {
                    if (track != null) {
                        if (track.getTrack().getIsPlayable() == null || track.getTrack().getIsPlayable()) {
                            tracksConverted.add(new tk.leoforney.requestit.model.spotify.Track(track.getTrack()));
                        }
                    }
                }
            }
